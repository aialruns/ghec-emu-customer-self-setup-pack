...
**Document for review:** GitHub Enterprise Cloud: Customer Self-Setup Guide  
**Review copy location:** `review-copy/github-enterprise-customer-self-setup-guide.md`  
**Purpose:** Collect structured feedback before broader circulation or formal publishing.

## Requested review focus

Please review this guide for:

- Technical accuracy of GitHub Enterprise Cloud with EMU setup steps
- Correctness of authentication and SCIM guidance
- Clarity of Azure billing setup instructions
- Appropriateness for first-time enterprise administrators
- Suitability for regulated, public sector, or government environments
- Missing prerequisites, validation checks, rollback steps, or operational caveats

## Reviewer questions

1. Are any setup steps unclear, risky, or incomplete for a first-time admin?
2. Are there any missing prerequisites, permissions, or stakeholder dependencies?
3. Are the pilot validation, rollback, and go-live readiness sections sufficient?
4. Are any references, caveats, or product limitations missing?
5. Should any sections be simplified, reordered, or made more explicit?

## Suggested review method

- If you are reviewing in GitHub, leave comments directly on the Markdown file or in a pull request.
- If you are reviewing offline, add comments in the PDF and return annotated feedback.
- If feedback is broad or cross-cutting, summarize it in a separate note and link to the relevant section.
...
- Team / function:
- Review date:
- Overall recommendation: Approve / Approve with edits / Needs revision
...
This guide helps you set up GitHub Enterprise Cloud (GHEC) with Enterprise Managed Users (EMU) using a self-service approach. It follows a controlled setup path that is practical for any first setup and emphasizes pilot validation before broader rollout.
...
| Hosting | `GHE.com` | You need GitHub data residency in a supported region | Confirm region choice and internal approval before rollout; track network allowlists and feature differences in your rollout plan |
...
- You have an approved credential vault, enterprise password manager, or managed secrets vault for recovery codes, SCIM tokens, and any SAML certificate details. The storage solution must meet your security requirements.
...
| Identity provider admin access (for example, Entra Cloud Application Administrator or equivalent) | Create the enterprise application, assign pilot users, and configure federation and SCIM | Your identity team |
...
4. Before broad rollout, run one controlled remove-or-unassign test for a noncritical pilot account. Verify the expected lifecycle result in GitHub, such as the account being deprovisioned, suspended, or otherwise handled per your identity design.
...
If any item is incomplete, resolve it before broader rollout. Do not expand beyond the pilot scope until the responsible admin, security, and operational stakeholders provide documented approval that the pilot met the required criteria.
...
| --- | --- | --- | --- | --- |
| SSO test fails or returns unauthorized | Confirm the pilot user is assigned to the app and has the correct role | Identity provider app assignments and GitHub auth test | Pilot user not assigned or wrong role | Correct assignment and rerun the test |
| SAML settings are rejected | Recheck every URL and entity value for exact format | Identity provider SAML settings and GitHub SAML page | URL format mismatch or trailing slash issue | Re-enter the identity values exactly as required |
| OIDC test fails | Recheck issuer and client values and confirm pilot assignment scope | Identity provider OIDC app settings and GitHub OIDC page | Issuer or client mismatch, or wrong user scope | Correct values or assignment scope and rerun |
| SCIM test connection fails | Validate the tenant URL and PAT scope first | Identity provider provisioning page and token details | Wrong tenant URL, wrong token scope, or expired PAT | Correct the tenant URL, token scope, or rotate the PAT |
| Users are not provisioning | Confirm provisioning was started and pilot scoping is correct | Identity provider provisioning logs and GitHub enterprise **People** | Provisioning not started, wrong scoping, or assignment issue | Start provisioning and verify pilot group scope |
| Managed user cannot sign in | Confirm the user is using the managed identity path, not a personal GitHub account | Pilot user sign-in flow and enterprise **People** | User is using the wrong username or wrong sign-in path | Retry with the managed identity entry point |
| Azure subscription cannot connect | Confirm Azure admin consent rights and target subscription access | Azure admin session and GitHub billing page | Missing tenant-wide admin consent or subscription access | Use an eligible Azure admin and retry |
| Copilot seat assigned but no IDE access | Confirm enterprise policy, team mapping, and pilot team membership | Copilot policy, enterprise team membership, and IDE sign-in | Team mapping incomplete or membership not applied yet | Verify team mapping and allow time for propagation |
